...
const RequestCard = ({ request, onViewDetails }) => {
  ...
  const formattedDate = new Date(request.preferred_date).toLocaleDateString('en-US', {
    year: 'numeric',
    month: 'short',
    day: 'numeric'
  });
  ...
  return (
    ...
    >
      ...
      {/* Waste Type and Priority Row */}
      <View className="flex-row justify-between items-center mb-3">
        <Text className="text-sm text-gray-600">{request.waste_type}</Text>
